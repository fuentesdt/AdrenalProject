...
idcols <- c("phase","necrosis", "MRN", "enhancement", "enhancement_difference","phenotype")
inputcols <- allinputs [grep("original_*",colnames(datamatrix))]
longformatmatrix<-  datamatrix[c(inputcols ,idcols) ]
dsraw  = reshape(longformatmatrix       ,  idvar = c( "necrosis", "MRN",  "enhancement_difference","phenotype" ), timevar = "phase"     , direction="wide")

if( length(widefeaturematrix$MRN) != length(unique(widefeaturematrix$MRN))){
stop("data massage error")
}
...
#HACK need clinical columns
if( 'MRN' %in% Filteredinputs) Filteredinputs=Filteredinputs[Filteredinputs!= 'MRN']
if(!'enhancement_difference' %in% Filteredinputs) Filteredinputs=c(Filteredinputs,'enhancement_difference')
if(!'necrosis' %in% Filteredinputs) Filteredinputs=c(Filteredinputs,'necrosis')
if(!'enhancement.pre' %in% Filteredinputs) Filteredinputs=c(Filteredinputs,'enhancement.pre')
if(!'enhancement.con' %in% Filteredinputs) Filteredinputs=c(Filteredinputs,'enhancement.con')
ds = dsraw [c(params$target,Filteredinputs)]
...
# Variable selection 
staginginputs = c("necrosis","enhancement.pre","enhancement.con","enhancement_difference") # model build with staging input only
variableSelections <- list(staging=staginginputs) #list(all=Filteredinputs)
